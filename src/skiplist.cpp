...
// 节点构造函数
SkipList::Node::Node(int level, double score, const SDS &member)
...
// 分数和成员比较函数（处理分数相同的情况）
int SkipList::compare(double score, const SDS &member, const Node *node) const
{
...
// 删除操作
bool SkipList::remove(const SDS &member)
{
    if (member.empty())
    {
        throw std::invalid_argument("Member cannot be empty");
...
// 获取成员分数
double SkipList::get_score(const SDS &member)
{
    if (member.empty())
    {
        throw std::invalid_argument("Member cannot be empty");
...
// 获取成员排名（从1开始）
int SkipList::get_rank(const SDS &member)
{
    if (member.empty())
    {
        throw std::invalid_argument("Member cannot be empty");
...
// 根据排名获取成员
SDS SkipList::get_by_rank(int rank)
{
...
// 获取所有数据
std::vector<std::pair<double, SDS>> SkipList::get_all_data()
{
...
// 根据分数范围查询
std::vector<std::pair<double, SDS>> SkipList::range(double min_score, double max_score)
{
    std::vector<std::pair<double, SDS>> result;
...
// 根据排名范围查询
std::vector<std::pair<double, SDS>> SkipList::range(int start, int stop)
{
    std::vector<std::pair<double, SDS>> result;
    // 验证排名范围是否有效
    // 处理负索引
